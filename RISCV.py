...
class TaskType(Enum):
    ...
    def cycles_per_operation(self):
        ...
        # Необходимо для имитаций различного времени выполнения у различных типов задач
        cycles = {
...
class RISCVProcessor:
            ...
            for cluster in range(self.clusters):  # Проходим по всем кластерам
                for core in range(self.cores_per_cluster):  # Проходим по всем ядрам в кластере
                    if self.core_status[cluster][core]:  # Если ядро свободно
                        ...
                        task.core = core  # Присваиваем задаче идентификатор ядра
                        task.cluster = cluster  # Присваиваем задаче идентификатор кластера
                        ...
                        self.logger.debug(
                            f"Allocated Task {task.id} to Cluster {cluster}, Core {core}"
                            f" Core {core} with execution time {task.execution_time:.6f} сек and max_lifetime {task.max_lifetime:.6f} сек")
                        return (cluster, core)  # Возвращаем кортеж с идентификаторами кластера и ядра
    ...
    def release_core(self, cluster: int, core: int):
        ...
        with self.core_lock:  # Блокируем доступ к ядрам для других потоков
            self.core_status[cluster][core] = True  # Помечаем ядро как свободное
...
class MultiprocessorRTOS:
    def __init__(self, amount_of_processors):
        ...
        # Создаём словарь процессоров, индексированных по их номеру
        self.processors = {
            idx: RISCVProcessor(idx)
        ...
        self.next_processor_id = 0  # Индекс следующего процессора для циклического распределения задач
    ...
    def _select_processor(self, task: Task) -> Optional[RISCVProcessor]:
        ...
        for _ in range(num_processors):  # Проходимся по всем процессорам
            ...
            self.next_processor_id = (self.next_processor_id + 1) % num_processors  # Обновляем индекс для следующего вызова
            ...
            with processor.core_lock:  # Блокируем доступ потокам к ядрам процессора
                for cluster in range(processor.clusters):  # Проходим по всем кластерам
                    for core in range(processor.cores_per_cluster):  # Проходим по всем ядрам в кластере
                        if processor.core_status[cluster][core]:  # Если ядро свободно
    ...
    def process_tasks(self, total_simulation_time=60):
        ...
        while time.time() - start_time < total_simulation_time:  # Пока не истекло время симуляции
            ...
            transfer_rate = 120e9  # Скорость передачи данных 120 Гбит/с
...
def generate_ieee_802_3ba_tasks(count=192):
    """
    ...
    for i in range(count):
        ...
        data_size = random.randint(10240, 1024000)  # Размер данных в битах (случайный из диапазона)
        ...
        task = Task(
            ...
            id=i,  # Уникальный идентификатор задачи
            data_size=data_size,
            type=task_type,
